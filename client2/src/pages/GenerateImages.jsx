...
const GenerateImages = () => {
  ...
  const [content, setContent] = useState(""); // will hold image url/base64

  const [imageLoaded,setImageLoaded]=useState(false); //it shows download option after completion complete complete image generation 
  ...
  const onSubmitHandler = async (e) => {
    ...
    try {
      ...
      if (data.success) {
        setContent(data.content); // backend should return image url or base64
      } else {
  ...
  // ✅ simple download handler without backend
  const handleDownload = async () => {
    try {
      ...
      link.href = url;
      link.download = "generated.png"; // 👈 This forces "Save As"
